...
# ----------------------------------------------------------------------
files = {
    "T=1.0 random":  "data_output/energy_T1_random.txt",
...
for label, path in files.items():
    dfs[label] = pd.read_csv(path, sep=r"\s+", names=["cycle", "E_inst", "E_mean"])

for name, df in dfs.items():
    print(name, "min/max of columns:")
    print(df[["E_inst", "E_mean"]].min(), "\n", df[["E_inst", "E_mean"]].max(), "\n")


plt.figure(figsize=(9,6))
for label, df in dfs.items():
    plt.plot(df["cycle"], df["E_inst"], "--", alpha=0.7, lw=1.2, label=label)
